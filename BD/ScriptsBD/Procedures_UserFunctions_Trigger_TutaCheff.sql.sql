...
exec SP_FREQ_INGRED (11); --Código de ingrediente inválido
exec SP_FREQ_INGRED (2); --Utilizado moderadamente
select * from tb_ingrediente; --verificar tb_ingrediente
select * from tb_relatorio_ingr; --verificar tabela com informações da proc
...
-- alterando uma receita acrescentando número nos modos de preparo para verificar a trigger
...
4 - Frite até o ponto que preferir.'where id_receita=4;

--verificando tabela da trigger após alteração da receita
...
begin
  ...
  IF V_QTDEINGRED < 2 THEN
        ROLLBACK; --Impede que insira a receita inválida
